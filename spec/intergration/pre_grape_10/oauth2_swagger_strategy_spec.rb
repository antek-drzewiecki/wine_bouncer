...
describe Api::PreGrape10::MountedSwaggerApiUnderTest, type: :api do
  ...
  context 'tokens and scopes' do
    it 'gives access when the token and scope are correct' do
      ...
      get '/pre_grape_10/swagger_api/protected', nil, 'HTTP_AUTHORIZATION' => "Bearer #{token.token}"
    ...
    it 'raises an authentication error when the token is invalid' do
      expect { get '/pre_grape_10/swagger_api/protected', nil, 'HTTP_AUTHORIZATION' => "Bearer #{token.token}-invalid" }.to raise_exception(WineBouncer::Errors::OAuthUnauthorizedError)
    end

    it 'raises an oauth authentication error when no token is given' do
      expect { get '/pre_grape_10/swagger_api/protected' }.to raise_exception(WineBouncer::Errors::OAuthUnauthorizedError)
    end

    it 'raises an auth forbidden authentication error when the user scope is not correct' do
      expect { get '/pre_grape_10/swagger_api/protected_with_private_scope', nil, 'HTTP_AUTHORIZATION' => "Bearer #{token.token}" }.to raise_exception(WineBouncer::Errors::OAuthForbiddenError)
    end
  end

  context 'unprotected endpoint' do
    it 'allows to call an unprotected endpoint without token' do
      get '/pre_grape_10/swagger_api/unprotected'
    ...
    it 'allows to call an unprotected endpoint with token' do
      get '/pre_grape_10/swagger_api/unprotected', nil, 'HTTP_AUTHORIZATION' => "Bearer #{token.token}"
  ...
  context 'protected_without_scopes' do
    ...
    it 'allows to call an protected endpoint without scopes' do
      get '/pre_grape_10/swagger_api/protected_without_scope', nil, 'HTTP_AUTHORIZATION' => "Bearer #{token.token}"
    ...
    it 'raises an error when an protected endpoint without scopes is called without token ' do
      expect { get '/pre_grape_10/swagger_api/protected_without_scope' }.to raise_exception(WineBouncer::Errors::OAuthUnauthorizedError)
    end

    it 'raises an error because the user does not have the default scope' do
      expect { get '/pre_grape_10/swagger_api/protected_without_scope', nil, 'HTTP_AUTHORIZATION' => "Bearer #{unscoped_token.token}" }.to raise_exception(WineBouncer::Errors::OAuthForbiddenError)
    end
  end

  context 'resource_owner' do
    it 'is available in the endpoint' do
      get '/pre_grape_10/swagger_api/protected_user', nil, 'HTTP_AUTHORIZATION' => "Bearer #{token.token}"
